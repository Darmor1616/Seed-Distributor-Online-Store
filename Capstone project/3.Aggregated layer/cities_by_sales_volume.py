...
SELECT 
    ...
    SUM(d.totalQuantity * i.price) AS TotalRevenue
FROM 
    fct_orders o
JOIN 
    dim_addresses a ON o.addressId = a.id
    and '{report_date}' between a.beg_date and a.exp_date 
JOIN 
    fct_orderdetails d ON o.id = d.orderId
JOIN 
    dim_items i ON d.itemId = i.id
    and '{report_date}' between i.beg_date and i.exp_date 
WHERE 
    o.deliveredOn IS NOT NULL
GROUP BY 
    a.city
''')
